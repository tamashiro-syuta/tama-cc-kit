---
name: small-flow
description: devflow の Small フロー。Router が small と判定したタスクを、implementer 1 人・レビュー 1 往復・人間の確認・コミット・PR で完了させる。devflow の run / resume Skill から呼ばれる。直接使用するものではない。
...
`status.py set phase '"human_review"'`。人間に示す: 変更ファイル、reviewer の non-blocking 指摘と残存リスク、実行したテストコマンド。コミットと PR 作成の承認を求め、待つ。修正依頼があれば `SESSION_DIR/tasks/T1.md` の「Human feedback」見出しの下に書き、次のラウンド番号でステップ 2 に戻る。これは許容ラウンド 1 回分として数える。
